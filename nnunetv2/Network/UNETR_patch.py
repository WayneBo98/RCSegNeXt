...
class SingleDeconv3DBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=2, stride=2, padding=0):
        super().__init__()
        self.block = nn.ConvTranspose3d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, output_padding=0)
...
class Deconv3DBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3, up_kernel_size=2, stride=2):
        super().__init__()
        self.block = nn.Sequential(
            SingleDeconv3DBlock(in_planes, out_planes, kernel_size=up_kernel_size, stride=stride),
...
class UNETR(nn.Module):
    def __init__(self, img_shape=(128, 128, 128), input_dim=4, output_dim=3, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        ...
        self.patch_dim = [int(img_shape[i] / patch_size[i]) for i in range(len(img_shape))]
        ...
        self.decoder3 = \
            nn.Sequential(
                Deconv3DBlock(embed_dim, 512, up_kernel_size=[1,2,2], stride=[1,2,2]),
                Deconv3DBlock(512, 256, up_kernel_size=[1,2,2], stride=[1,2,2]),
        ...
        self.decoder12_upsampler = \
            SingleDeconv3DBlock(embed_dim, 512, kernel_size=[1,2,2], stride=[1,2,2])
        ...
        self.decoder9_upsampler = \
            nn.Sequential(
                ...
                Conv3DBlock(512, 512),
                Conv3DBlock(512, 512),
                SingleDeconv3DBlock(512, 256, kernel_size=[1,2,2], stride=[1,2,2])
            )
