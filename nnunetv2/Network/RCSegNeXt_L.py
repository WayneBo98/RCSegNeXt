...
def add_conv_stage_next(dim_in, dim_out, kernel_size=3, stride=1, padding=1, exp_r=2, num_blocks=2):
    layers = [ConvNeXtBlock(dim_in, dim_out, kernel_size=kernel_size, stride=stride, padding=padding, exp_r=exp_r)]
    for _ in range(num_blocks - 1):
        layers.append(ConvNeXtBlock(dim_out, dim_out, kernel_size=kernel_size, stride=1, padding=padding, exp_r=exp_r))
    return nn.Sequential(*layers)

def add_conv_stage_nest(dim_in, dim_out, kernel_size=3, stride=1, padding=1, exp_r=2, scale=4, num_blocks=2):
    layers = [ConvNeStBlock(dim_in, dim_out, kernel_size=kernel_size, stride=stride, padding=padding, exp_r=exp_r, scale=scale)]
    for _ in range(num_blocks - 1):
        layers.append(ConvNeStBlock(dim_out, dim_out, kernel_size=kernel_size, stride=1, padding=padding, exp_r=exp_r, scale=scale))
    return nn.Sequential(*layers)
class UNet(nn.Module):
    def __init__(self, in_channel=1, num_class=3,channel=48,scale=4):
        ...
        self.encoder4 =add_conv_stage_nest(channel*4, channel*8, scale = scale, num_blocks=8)
        self.encoder5 = add_conv_stage_nest(channel*8, channel*16, scale = scale, num_blocks=4)
        # self.encoder5=   nn.Conv3d(256, 512, 3, stride=1, padding=1)

        # self.decoder1 = nn.Conv3d(512, 256, 3, stride=1,padding=1)  # b, 16, 5, 5
        self.decoder4 = add_conv_stage_nest(channel*16, channel*8,scale=scale, num_blocks=8)  # b, 8, 15, 1
